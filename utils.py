...
def groupingByYear(df: pd.DataFrame) -> pd.Series:
    ...
    """
    return df[df.Year_of_Release < 2017].Year_of_Release.groupby(df.Year_of_Release).count()
...
def groupingByYearCount(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    """
    return df[df.Year_of_Release < 2017].pivot_table(
...
def groupingByYearCountPercetange(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    dfYearCountPercentage = dfYearCount.drop('Year_of_Release', axis=1).div(YearCountSum, axis=0)
    ...
    dfYearCountPercentage = pd.concat([dfYearCount[['Year_of_Release']], dfYearCountPercentage], axis=1)
...
def groupingByYearSales(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    return df[df.Year_of_Release < 2017].pivot_table(
        index='Year_of_Release', columns=column, values='Global_Sales', aggfunc='sum', fill_value=0
    ).reset_index()
...
def groupingByYearSalesPercetange(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    dfYearSalesPercentage = dfYearSales.drop('Year_of_Release', axis=1).div(YearSalesSum, axis=0)

    # Multiply by 100 to get percentages, concatenate with the 'Year_of_Release' column, and return the result
    dfYearSalesPercentage *= 100
    dfYearSalesPercentage = pd.concat([dfYearSales[['Year_of_Release']], dfYearSalesPercentage], axis=1)
...
def groupingSumByYear(df: pd.DataFrame) -> pd.Series:
    ...
    """
    return df[df.Year_of_Release < 2017]['Global_Sales'].groupby(df.Year_of_Release).sum()
...
def getKPIs(df: pd.DataFrame, column: str) -> list:
    ...
    columnSum = df[df.Year_of_Release < 2017][['Global_Sales']].groupby(df[column]).sum().reset_index()
    ...
    keys = ['Most Successful ' + column, bestSeller + ' ' + column + ' Total Sales', 'Best out of']
